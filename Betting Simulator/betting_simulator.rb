require 'pg'               #PostgreSQL gem for connecting to a database
...
class Simulator
    ...
    def outcome(num)
        if num.even?                                #even number results in a win
            ...
            self.num_wins += 1
        else                                        #odd number results in a loss
    ...
    def upload(profit, win_percentage)
        begin
            con = PG.connect :dbname => 'db_name', :user => 'username'                                                                          #connects to db
            con.exec "INSERT INTO table_name (base_wager, win_percentage, profit) VALUES (#{self.base_wager}, #{win_percentage}, #{profit})"    #inserts new row of data into table
            data = con.exec "SELECT * FROM table_name"
            puts
            data.each { |row| puts "%s %s %s %s" % [ row['id'], row['base_wager'], row['win_percentage'], row['profit'] ] }                     #prints each row of the table
...
Simulator.new(10000, 1)     #runs the sim with 10000 bets and a base_wager of 1